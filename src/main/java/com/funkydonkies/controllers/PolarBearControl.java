...
public class PolarBearControl extends GhostControl implements PhysicsCollisionListener {
	 ...
	 * 
	 * @param mass
	 *            the mass of the spatial
	 * @param sp
	 *            the constant speed for the spatial
	 * @param moveHor
	 *            a boolean to check if the spatial moves horizontal or vertical
	 * @param moveUpRight
	 *            a boolean to check if the spatial moves right or left
	 */
	public PolarBearControl(final CollisionShape shape, float stopX, final double sp,
			AppStateManager asm, Vector3f loci) {
		super(shape);
		sm = asm;
		this.speed = sp;
		initialLoc = loci;
