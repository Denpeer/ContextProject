...
public class SplineCurve extends Spline {
	 ...
	 *            default material
	 * @param splineType
	 *            the type of the SplineCurve, in our case Catmulrom
	 * @param cycle
	 *            the cycle of the curve
	 */
	 ...
	 * @param node
	 *            the rootNode everything gets attached too
	 */
	public final void drawCurve(final PhysicsSpace physicsSpace, final RigidBodyControl rigidBody,
			final Node node) {
		phys = rigidBody;
	...
	public Vector3f[] getSplinePoints() {
		...
		for (int i = 2; i < curvePoints.length - 2; i++) {
			for (double j = 0; j < SEGMENTPOINTS_PER_CONTROLPOINT * SCALE_POINTS; j = j + OFFSET
					* SCALE_POINTS) {
	 ...
	 * 
	 * @param points
	 *            desired Vector3f[] containing the points the curve uses to create itself.
	 */
