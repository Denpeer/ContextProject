...
public class FishControlTest {
	...
	public void setUp() throws Exception {
		...
		Mockito.when(event1.getNodeB()).thenReturn(spatial);
		Mockito.when(spatial.getName()).thenReturn(FishFactory.FISH_NAME);
		Mockito.when(spatial.getName()).thenReturn(PenguinFactory.PENGUIN_NAME);
		Mockito.when(event2.getNodeA()).thenReturn(spatial2);
		Mockito.when(event2.getNodeB()).thenReturn(spatial2);
		Mockito.when(spatial2.getName()).thenReturn(FishFactory.FISH_NAME);
		Mockito.when(spatial2.getName()).thenReturn(SnowballPowerup.SNOW_PENGUIN_NAME);
	}
